...
| `core/input.lua` | Central rebindable key map for the single-player gameplay actions (movement, strafe/turret modifier, fire, takeoff/land, cycle weapon, pause). Each action holds a list of keys (defaults keep both WASD and arrows); `held`/`pressed` query it, `rebind` sets a single key, and it persists to `data/keybinds.json`. The gameplay scene points the player's `controls` at this map and matches the action keys; the `CONTROLS` page of the advanced options edits it live. Co-op keeps its own per-player key sets and does not use this. |
