...
- **replays** (`F4`): the replay browser. Lists the recordings in the save
  directory and plays one back (`Enter`), verifies one at 20x against its
  checksums (`V`), toggles recording of new runs (`R`), or deletes one (`X`).
  See `DETERMINISM.md`.
...
| `core/rng.lua` | Seeded simulation randomness (`world.rng`, a `love.math` generator created per phase from the run seed) plus a draw counter the replay checksum folds in. Everything the simulation rolls draws from it; presentation (draw functions, the weather field) keeps the global `math.random` so rendering can never move the simulation stream. |
| `core/input_frame.lua` | One tick's input for one player: a held-action bitmask (`up`/`down`/`left`/`right`/`modifier`/`fire`, a fixed bit order that is part of the replay format) plus the edge actions that fired (`takeoff`, `weapon`, `god`, `pickup_mode`, `slot:N`). The only way input reaches the simulation. |
| `core/input_source.lua` | Where a tick's frames come from: `Local` samples the keyboard once per tick and drains the events `love.keypressed` queued, `Replay` returns recorded frames. Single player builds one, co-op two, playback one per recorded slot; a network peer would be a third implementation. |
| `dev/selftest.lua` | Determinism self-test (`love . --selftest [ticks] [stage]`): runs a scripted phase three ways (the same script twice, record-then-replay, and record-then-replay through a pause) and compares the simulation tick by tick, naming the first tick that differs and the fields that moved. The regression tool for anything replay-related. |
| `game/replay.lua` | Recorded run: header (build, stage, mode, seed, simulation params, per-player vehicle / loadout / ammo), delta encoded per-tick input, and a state checksum every 60 ticks. Also the file format (text, `replays/*.osr` in the save directory), the listing used by the picker, and `Replay.checksum`, which folds player kinematics, entity health and state, the projectile count and the RNG draw count into one number for divergence detection. |
...
game). `F4` opens the replay browser (play back or verify a recorded run). `F8` opens the animation gallery: a test overlay that plays every clip in
