...
| `core/assets.lua` | Resolves a data-driven asset path to one of the two roots: `content/...` for engine-owned artwork shipped with the build, anything else for the `assets/` pack decoded from the user's game files. |
...
| `game/postfx.lua` | Gameplay post-processing over the world view (HUD excluded): colour grade, S-curve contrast, sharpen, quarter-size bloom, per-viewport vignette and grain in one composite shader, plus ground shadows (aircraft bodies, and with soft shadows on their rotors, helicopter damage smoke, and the missiles and trail puffs of `shadow`-flagged weapons) merged into their own target with a max blend and blurred by repeated narrow gaussian passes. Look constants and presets in `data/postfx.json`. See `EXTRA.md`. |
...
| `game/shadow.lua` | Ground shadows: one light model plus a helper that masks a sprite to a flat silhouette, scaled by altitude, skipped on night missions. Aircraft bodies always cast; rotors, helicopter damage smoke, missiles and their trails only inside the soft-shadow pass. |
...
window or the camera. A weapon flagged `shadow` (the missiles) casts ground
shadows from its projectile sprite and trail puffs, drawn by
`CombatSystem:draw_shadows` only while soft shadows are on.
...
| `data/hud.json` | HUD layout and gauge sprites. Sprite paths resolve through `core/assets.lua`: bare paths come from the decoded pack, `content/` paths from the shipped artwork (the score readout's `icon_pattern` is the P1-P4 player label). |
...
| `data/postfx.json` | Post-processing look: `look` holds what 100% of each strength means (exposure, warmth, saturation, contrast, sharpen, bloom threshold / gain / tint / spread, vignette amount / power, grain, shadow blur spread / pass count / gain); `presets` is the ordered NONE / MILD / FULL list of strength values. |
...
The engine-owned artwork in `content/` needs none of the above and is committed:
`python3 tools/gen_hud_labels.py` regenerates the P1-P4 HUD score labels.
